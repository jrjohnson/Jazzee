...
/**
 * Custom hydrator for applicant records the
 * loads all of the answer data from each
 * page and element class
 *
 */
class ApplicantArray extends \Doctrine\ORM\Internal\Hydration\ArrayHydrator
{
    ...
    public function hydrateAll($stmt, $resultSetMapping, array $hints = array())
    {
        ...
        foreach ($result as $key => $applicant) {
            ...
            foreach ($applicant['attachments'] as $attKey => $attachment) {
                if (array_key_exists('answer_id', $attachment) and
                    !empty($attachment['answer_id'])
                ) {
                    unset($applicant['attachments'][$attKey]);
                }
            }
