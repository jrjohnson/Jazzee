...
use Jazzee\CommonBundle\Entity;

/**
 * ApplicationRepository
 * Special Repository methods for Application to make searchign 
 * for special conditions easier
 *
 * @author  Jon Johnson  <jon.johnson@ucsf.edu>
 * @license http://jazzee.org/license BSD-3-Clause
 */
class Application extends \Doctrine\ORM\EntityRepository
{
     ...
     * @return Application
     */
    public function findOneByProgramAndCycle(Entity\Program $program, Cycle $cycle)
    {
        $query = $this->_em->createQuery(
            'SELECT a FROM Jazzee\CommonBundle\Entity\Application a WHERE ' .
            'a.program = :programId AND  a.cycle = :cycleId'
        );
    ...
    public function findByProgram(
        ...
        $queryBuilder->add('select', 'a')
                ->from('Jazzee\CommonBundle\Entity\Application', 'a');
     ...
     * @return array
     */
    public function getPageAnswerCounts(Entity\Application $application)
    {
        $query = $this->_em->createQuery(
            'SELECT page.id as pageId, count(answer.id) as answers ' .
            'FROM Jazzee\CommonBundle\Entity\Answer answer JOIN answer.page page JOIN ' .
            'answer.applicant applicant WHERE answer.applicant IN ' .
            '(SELECT app1.id FROM Jazzee\CommonBundle\Entity\Applicant app1 WHERE ' .
            'app1.application = :applicationId) GROUP BY ' .
            'answer.applicant, answer.page'
        );
        $query->setParameter('applicationId', $application->getId());
        $pageAnswers = array();
        foreach ($query->getResult() as $arr) {
            if (!array_key_exists($arr['pageId'], $pageAnswers) or
        ...
        foreach ($application->getApplicationPages() as $applicationPage) {
            if (array_key_exists(
                ...
                $pages[$applicationPage->getPage()->getId()] =
                    $pageAnswers[$applicationPage->getPage()->getId()];
            } else {
     ...
     * @return Application
     */
    public function findByProgramAndCycleName($programShortName, $cycleName)
    {
        $query = $this->_em->createQuery(
            'SELECT a FROM Jazzee\CommonBundle\Entity\Application a WHERE a.program = ' .
            '(SELECT p FROM Jazzee\CommonBundle\Entity\Program p WHERE ' .
            'p.shortName = :programShortName) AND  a.cycle = ' .
            '(SELECT c FROM \Jazzee\CommonBundle\Entity\Cycle c WHERE c.name= :cycleName)'
        );
    ...
    {
        $query = $this->_em->createQuery(
            'SELECT a FROM Jazzee\CommonBundle\Entity\Application a JOIN ' .
            'a.applicants applicant WHERE applicant.id = :applicantId'
        );
    ...
    public function findArray($id)
    {
        ...
        if ($result = $query->getArrayResult()) {
            ...
            $query = $this->_em->createQuery(
                'SELECT tag from \Jazzee\CommonBundle\Entity\Tag as tag LEFT JOIN ' .
                'tag.applicants applicant WHERE applicant.id IN ' .
    ...
    {
        $queryBuilder = $this->_em->createQueryBuilder();
        $queryBuilder->from('Jazzee\CommonBundle\Entity\Application', 'application');
        $queryBuilder->add(
            'select',
            'application, applicationPages, pages, elements, ' .
            'elementListItems, pageType, elementType, children, ' .
            'childElements, childPageType, childElementListItems, ' .
            'childElementType, children2, childElements2, ' .
            'childPageType2, childElementListItems2, ' .
            'childElementType2, pdfTemplates'
        );
        ...
        $queryBuilder->leftJoin(
            'childElements2.listItems',
            'childElementListItems2'
        );
