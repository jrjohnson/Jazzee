 ...
 */
class Applicant extends \Doctrine\ORM\EntityRepository
{
    ...
    public function findByCycle(Cycle $cycle)
    {
        $query = $this->_em->createQuery(
            'SELECT a FROM Jazzee\CommonBundle\Entity\Applicant a WHERE a.application IN ' .
            '(SELECT app FROM Jazzee\CommonBundle\Entity\Application app WHERE ' .
            'app.cycle = :cycleId) AND a.deactivated=false'
        );
    ...
    {
        $query = $this->_em->createQuery(
            'SELECT a FROM Jazzee\CommonBundle\Entity\Applicant a WHERE a != :applicantId ' .
            'AND a.email = :email AND a.application IN (SELECT app FROM ' .
            'Jazzee\CommonBundle\Entity\Application app WHERE app.cycle = :cycleId) ' .
            'AND a.deactivated=false'
        );
        $query->setParameter('applicantId', $applicant->getId());
        $query->setParameter(
            'cycleId',
            $applicant->getApplication()->getCycle()->getId()
        );
     ...
     * @return Application
     */
    public function findApplicantsByName(
        $firstName,
        $lastName,
        ...
        $queryBuilder->add('select', 'a')
                ->from('Jazzee\CommonBundle\Entity\Applicant', 'a');
    ...
    public function findByApplication(Application $application, $deep = false)
    {
        if (!$deep) {
            $queryBuilder = $this->_em->createQueryBuilder();
            $queryBuilder->from('Jazzee\CommonBundle\Entity\Applicant', 'applicant');
     ...
     * @return array
     */
    public function findIdsByApplication(
    ...
    public function find(
        $id,
        $deep = false,
        \Jazzee\Interfaces\Display $display = null
    ) {
        if (!$deep) {
            $queryBuilder = $this->_em->createQueryBuilder();
            $queryBuilder->from('Jazzee\CommonBundle\Entity\Applicant', 'applicant');
            ...
            if (!$display) {
                $display = new \Jazzee\Display\FullApplication(
                    $this->_em->getRepository('\Jazzee\CommonBundle\Entity\Application')
                    ->findForApplicant($id)
                );
            }
            $queryBuilder = $this->deepApplicantQuery($display);
    ...
    public function findArray($id, \Jazzee\Interfaces\Display $display = null)
    {
        ...
        $applicantArray = $query->getSingleResult('ApplicantArray');
     ...
     * @return array
     */
    public function findDisplayArrayByApplication(
        Application $application,
        \Jazzee\Interfaces\Display $display,
        array $applicantIds
    ) {
        $results = array();
        foreach (array_chunk($applicantIds, 20) as $limitedIds) {
            ...
            foreach ($limitedIds as $key => $value) {
                ...
                $expression->add(
                    $queryBuilder->expr()->eq("applicant.id", ":{$paramKey}")
                );
            ...
            $query->setHydrationMode('ApplicantDisplay');
     ...
     * @return array
     */
    public function findPDFTemplateArrayByApplication(
        Application $application,
        \Jazzee\Interfaces\Display $display,
        array $applicantIds
    ) {
        ...
        foreach (array_chunk($applicantIds, 20) as $limitedIds) {
            ...
            $query->setHydrationMode('ApplicantPDFTemplate');
            $results = array_merge($results, $query->execute());
        }
    ...
    protected function deepApplicantQuery(\Jazzee\Interfaces\Display $display)
    {
        ...
        $queryBuilder->add(
            'select',
            'applicant, attachments, decision, tags, answers, ' .
            'element_answers, publicStatus, privateStatus, payment, ' .
            'answer_greScore, answer_toeflScore, answer_school, children, ' .
            'answer_attachment, children_element_answers, ' .
            'children_publicStatus, children_privateStatus, ' .
            'children_payment, children_attachment, children2, ' .
            'children_element_answers2, children_publicStatus2, ' .
            'children_privateStatus2, children_payment2, children_attachment2'
        );
        $expression = $queryBuilder->expr()->orX();
        //this one is the default - if there are no pages
        //in the display then this
        ...
        $expression = $queryBuilder->expr()->orX();
        //this one is the default - if there are no elements in the display
        //then this expression is the only one that will load and no
        //elements will be loaded
        $expression->add(
            $queryBuilder->expr()->eq("element_answers.element", ":nothing")
        );
        $queryBuilder->setParameter('nothing', 'nothing');
        foreach ($display->getElementIds() as $key => $elementId) {
            ...
            $expression->add(
                $queryBuilder->expr()->
                eq("element_answers.element", ":{$paramKey}")
            );
        ...
        $queryBuilder->leftJoin(
            'children.elements',
            'children_element_answers'
        );
        $queryBuilder->leftJoin(
            'children.publicStatus',
            'children_publicStatus'
        );
        $queryBuilder->leftJoin(
            'children.privateStatus',
            'children_privateStatus'
        );
        ...
        $queryBuilder->leftJoin(
            'children2.elements',
            'children_element_answers2'
        );
        $queryBuilder->leftJoin(
            'children2.publicStatus',
            'children_publicStatus2'
        );
        $queryBuilder->leftJoin(
            'children2.privateStatus',
            'children_privateStatus2'
        );
     ...
     */
    public function findApplicantsByQuery(
        \stdClass $obj,
        \Jazzee\Controller $controller,
        Application $application = null
    ) {
        $queryBuilder = $this->_em->createQueryBuilder();
        $queryBuilder->add('select', 'a')
        ...
        if (isset($obj->applicant)) {
            ...
            foreach (array('lastLogin', 'createdAt', 'updatedAt') as $name) {
                if (isset($obj->applicant->$name) and
                    !is_null($obj->applicant->$name)
                ) {
                    $queryBuilder->andWhere("a.{$name} >= :{$name}");
                    $queryBuilder->setParameter($name, $obj->applicant->$name);
                }
            }
            if (isset($obj->applicant->isLocked) and
                !is_null($obj->applicant->isLocked)
            ) {
        ...
        if (isset($obj->decision)) {
            ...
            foreach ($arr as $name) {
                if (isset($obj->decision->$name) and
                    $obj->decision->$name == true
                ) {
        ...
        foreach ($obj->pages as $pageObj) {
            foreach ($applicants as $key => $applicant) {
                if ($applicationPage = $applicant->getApplication()
                        ->getApplicationPageByTitle($pageObj->title)
                ) {
                    $applicationPage->getJazzeePage()->setApplicant($applicant);
                    $applicationPage->getJazzeePage()
                        ->setController($controller);
                    if (!$applicationPage->getJazzeePage()
                            ->testQuery($pageObj->query)
                    ) {
    ...
    {
        $query = $this->_em->createQuery(
            'UPDATE Jazzee\CommonBundle\Entity\Applicant a SET a.failedLoginAttempts = 0 ' .
            'WHERE a.failedLoginAttempts > 0'
        );
        $query->execute();
    }
    ...
    {
        $query = $this->_em->createQuery(
            'UPDATE Jazzee\CommonBundle\Entity\Applicant a SET a.uniqueId = null WHERE ' .
