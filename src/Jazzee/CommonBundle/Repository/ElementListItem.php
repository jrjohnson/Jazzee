 ...
 */
class ElementListItem extends \Doctrine\ORM\EntityRepository
{
    ...
    public function search(Element $element, $searchTerms, $variables = array())
    {
        ...
        $queryBuilder->add('select', 'item')
                ->from('Jazzee\CommonBundle\Entity\ElementListItem', 'item')
        ...
        foreach ($searchTerms as $key => $term) {
            $expression2->add(
                $queryBuilder->expr()->like("item.value", ":term{$key}")
            );
        ...
        foreach ($searchTerms as $key => $term) {
            ...
            foreach ($variables as $key2 => $name) {
                $expression3->add(
                    $queryBuilder->expr()->eq("variable.name", ":v{$key2}")
                );
                $expression3->add(
                    $queryBuilder->expr()->like("variable.value", ":term{$key}")
                );
