 ...
 *
 */
class School extends \Doctrine\ORM\EntityRepository
{
     ...
     * @param string $string
     * 
     * @return array \Jazzee\CommonBundle\Entity\School
     */
    public function search($string)
    {
        $queryBuilder = $this->_em->createQueryBuilder();
        $queryBuilder->add('select', 'school')->from('Jazzee\CommonBundle\Entity\School', 'school');
    ...
    {
        $queryBuilder = $this->_em->createQueryBuilder();
        $queryBuilder->add('select', 'count(school.id)')->from('Jazzee\CommonBundle\Entity\School', 'school');
        $searchTerms = explode(' ', $string);
        $queryBuilder->andWhere($this->getSearchExpression($searchTerms));
        foreach ($searchTerms as $key => $value) {
            $queryBuilder->setParameter("term{$key}", '%' . $value . '%');
    ...
    {
        $query = $this->_em->createQuery('SELECT COUNT(s.id) FROM Jazzee\CommonBundle\Entity\School s');
