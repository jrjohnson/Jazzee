 ...
 */
class TOEFLScore extends \Doctrine\ORM\EntityRepository
{
    ...
    public function getStatistics()
    {
        ...
        $query = $this->_em->createQuery(
            'SELECT count(t) as Total FROM Jazzee\CommonBundle\Entity\TOEFLScore t'
        );
    ...
    public function findByName($firstName, $lastName)
    {
        ...
        $queryBuilder->add('select', 's')->
                from('Jazzee\CommonBundle\Entity\TOEFLScore', 's');
    ...
    {
        $query = $this->_em->createQuery(
            'SELECT CONCAT(CONCAT(s.registrationNumber, s.testMonth),' .
            's.testYear) as uniqueIdentifier, s.id ' .
            'FROM Jazzee\CommonBundle\Entity\TOEFLScore s'
        );
        $results = array();
    ...
    {
        $query = $this->_em->createQuery(
            'UPDATE Jazzee\CommonBundle\Entity\Answer a SET a.toeflScore = :scoreId WHERE ' .
            'a.id = :answerId'
        );
        $result = $query->execute(
            array('answerId' => $answerId, 'scoreId'  => $scoreId)
        );
        return $result;
    }
    ...
    public function pruneUnmatchedScores(\DateTime $olderThan)
    {
        ...
        $query = $this->_em->createQuery(
            'SELECT DISTINCT score.id FROM Jazzee\CommonBundle\Entity\Answer a ' .
            'JOIN a.toeflScore score WHERE score IS NOT NULL'
        );
        $existingIds = array();
        ...
        $query = $this->_em->createQuery(
            'DELETE FROM \Jazzee\CommonBundle\Entity\TOEFLScore s WHERE s.id NOT IN ' .
