...
class Payment extends \Doctrine\ORM\EntityRepository
{
     ...
     * @param string $status
     * @param \Jazzee\CommonBundle\Entity\PaymentType $paymentType
     * 
     * @return array
     */
    public function findIdByStatusAndTypeArray(
        $status,
        \Jazzee\CommonBundle\Entity\PaymentType $paymentType
    ) {
        $dql = 'SELECT p.id FROM \Jazzee\CommonBundle\Entity\Payment p WHERE ' .
            'p.type = :paymentType AND p.status=:status';
