 ...
 */
class GREScore extends \Doctrine\ORM\EntityRepository
{
    ...
    public function getStatistics()
    {
        ...
        $query = $this->_em->createQuery(
            'SELECT count(g) as Total FROM Jazzee\CommonBundle\Entity\GREScore g'
        );
        $result = $query->getResult();
        $return['total'] = $result[0]['Total'];

        $query = $this->_em->createQuery(
            "SELECT DISTINCT(CONCAT(CONCAT(g.testYear,'-'),g.cycleNumber) " .
            "as Cycle FROM Jazzee\CommonBundle\Entity\GREScore g ORDER BY Cycle DESC"
        );
    ...
    {
        $query = $this->_em->createQuery(
            'SELECT s FROM Jazzee\CommonBundle\Entity\GREScore s WHERE s.firstName LIKE ' .
            ':firstName AND s.lastName LIKE :lastName order by ' .
    ...
    {
        $dql = "SELECT CONCAT(CONCAT(s.registrationNumber, ' .
        's.testMonth),s.testYear) as uniqueIdentifier, ' .
        's.id FROM Jazzee\CommonBundle\Entity\GREScore s";
    ...
    {
        $query = $this->_em->createQuery(
            'UPDATE Jazzee\CommonBundle\Entity\Answer a ' .
            'SET a.greScore = :scoreId WHERE a.id = :answerId'
        );
    ...
    public function pruneUnmatchedScores(\DateTime $olderThan)
    {
        ...
        $query = $this->_em->createQuery(
            'SELECT DISTINCT score.id FROM Jazzee\CommonBundle\Entity\Answer a JOIN ' .
            'a.greScore score WHERE score IS NOT NULL'
        );
        $existingIds = array();
        ...
        $query = $this->_em->createQuery(
            'DELETE FROM \Jazzee\CommonBundle\Entity\GREScore s WHERE s.id NOT IN ' .
            "({$existingIds}) AND s.testDate < :olderThan"
        );
