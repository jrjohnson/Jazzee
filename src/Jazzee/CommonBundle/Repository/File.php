...
class File extends \Doctrine\ORM\EntityRepository
{
    ...
    public function deleteUnreferencedFiles()
    {
        $query = $this->_em->createQuery(
            'DELETE FROM Jazzee\CommonBundle\Entity\File f WHERE f.referenceCount = 0'
        );
        $query->execute();
    }
     ...
     * @return int
     */
    public function seedFileCache(
        \Jazzee\FileStore $fileStore,
        \DateTime $lastAccessedSince
    ) {
        $query = $this->_em->createQuery(
            'SELECT f.hash FROM Jazzee\CommonBundle\Entity\File f ' .
            'WHERE f.lastAccess > :lastAccess'
        );
    ...
    {
        $query = $this->_em->createQuery(
            'SELECT f from Jazzee\CommonBundle\Entity\File f WHERE f.hash= :hash'
        );
        ...
        $query = $this->_em->createQuery(
            'UPDATE Jazzee\CommonBundle\Entity\File f SET f.lastAccess = :lastAccess ' .
            'WHERE f.hash= :hash'
        );
        $query->setParameter('hash', $hash);
