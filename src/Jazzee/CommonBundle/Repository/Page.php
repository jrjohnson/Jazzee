...
class Page extends \Doctrine\ORM\EntityRepository
{
    ...
    public function hasAnswers(Page $page)
    {
        $query = $this->_em->createQuery(
            'SELECT a.id FROM Jazzee\CommonBundle\Entity\Answer a WHERE a.page = :pageId'
        );
    ...
    {
        $query = $this->_em->createQuery(
            'SELECT answer.id FROM Jazzee\CommonBundle\Entity\Answer answer ' .
            'JOIN answer.applicant applicant JOIN ' .
            'applicant.application application WHERE ' .
            'answer.page = :pageId AND application.cycle = :cycleId'
        );
        $query->setParameter('pageId', $page->getId());
    ...
    {
        $query = $this->_em->createQuery(
            'SELECT COUNT(answer.id) as ancnt FROM ' .
            'Jazzee\CommonBundle\Entity\Answer answer JOIN answer.applicant applicant ' .
            'WHERE answer.page = :pageId AND ' .
