...
class Element
{
     ...
     */
    public function addItem(\Jazzee\CommonBundle\Entity\ElementListItem $item)
    {
        $this->listItems[] = $item;
     ...
     * @return \Jazzee\CommonBundle\Entity\ElementListItem
     */
    ...
    public function getJazzeeElement()
    {
        if (is_null($this->jazzeeElement)) {
            ...
            if (!($this->jazzeeElement instanceof \Jazzee\Interfaces\Element)) {
                throw new \Exception(
                    $this->type > getName() . ' has class ' . $className .
                    ' that does not implement Element interface'
                );
            }
        }
