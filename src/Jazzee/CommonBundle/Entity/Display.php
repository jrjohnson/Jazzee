...
class Display
{
    ...
    public function __construct($type)
    {
        if (!in_array($type, array('user', 'role'))) {
            throw new \Exception("{$type} is not a valid type for Display");
        }
        $this->type = $type;
    ...
    public function setUser(User $user)
    {
        if (!in_array($this->type, array('user'))) {
            throw new \Exception(
                "You cannot set user for Displays that do not " .
                "have the type 'user'"
            );
        }
        $this->user = $user;
    ...
    public function setRole(Role $role)
    {
        if (!in_array($this->type, array('role'))) {
            throw new \Exception(
                "You cannot set role for Displays " .
                "that do not have the type 'role'"
            );
        }
        if ($role->isGlobal()) {
            throw new \Exception("Global roles cannot be used for displays");
        }
        if (isset($this->application) and
            $this->application->getProgram()->getId() !=
            $role->getProgram()->getId()
        ) {
            throw new \Exception("Role program / display program mismatch");
        }
        $this->role = $role;
    }
    ...
    {
        if (isset($this->role) and
            $this->role->getProgram()->getId() !=
    ...
    public function listElements()
    {
        $elements = array();
        foreach ($this->elements as $displayElement) {
            $elements[] = new \Jazzee\Display\Element(
                $displayElement->getType(),
                $displayElement->getTitle(),
                $displayElement->getWeight(),
                $displayElement->getName(),
                $displayElement->getPage() ?
                $displayElement->getPage()->getId() : null
            );
        }
