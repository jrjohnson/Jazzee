 ...
 * Sets the unique preferences for a givien 
 * Cycle/Program and contains all of its Applicants
 *
...
class Application
{
     ...
     * @param boolean $override publish without checking?
     */
    public function publish($override = false)
    {
        if (!$this->canPublish() or (!$override and !$this->shouldPublish())) {
            throw new \Exception(
                'Application cannot be published, it is not ready.'
            );
        }
        $this->published = true;
    }
    ...
    /**
     * Check if application is ready to be published 
     * or if there are non - blocking errors
     ...
     * @return array  \Jazzee\CommonBundle\Entity\ApplicationPage
     */
    ...
    public function compareWith(Application $application)
    {
        ...
        $thisPages = array();
        $applicationPages = $this->getApplicationPages(
        ...
        $otherPages = array();
        $applicationPages = $application->getApplicationPages(
            ApplicationPage::APPLICATION
        );
        foreach ($applicationPages as $applicationPage) {
        ...
        foreach ($thisPages as $title => $applicationPage) {
            ...
            if ($applicationPage->getPage()->isGlobal()) {
                $pageTitle .= '(' .
                    $applicationPage->getPage()->getTitle() .
                    ')';
            }
            if (!array_key_exists($title, $otherPages)) {
                $differences['pages']['new'][] = $pageTitle;
            } elseif ($applicationPage->getPage()->getType()->getId() !=
                    $otherPages[$title]->getPage()->getType()->getId()
            ) {
                $differences['pages']['new'][] = $pageTitle;
                if ($otherPages[$title]->getPage()->isGlobal()) {
                    $pageTitle = $title .
                        '(' . $otherPages[$title]->getPage()->getTitle() . ')';
                }
                $differences['pages']['removed'][] = $pageTitle;
            } elseif ($applicationPage->getPage()->isGlobal() and
                    $applicationPage->getPage()->getId() !=
                    $otherPages[$title]->getPage()->getId()
            ) {
                //different global pages
                $differences['pages']['new'][] = $title . '(' .
                    $applicationPage->getPage()->getTitle() . ')';
                $differences['pages']['removed'][] = $title .
                    '(' . $otherPages[$title]->getPage()->getTitle() . ')';
            } else {
                $pageDifferences = $applicationPage->getJazzeePage()
                    ->compareWith($otherPages[$title]);
    ...
    public function formatApplicantArray(array $applicant)
    {
        ...
        foreach ($answers as $pageId => $answers) {
            if ($applicationPage =
                $this->getApplicationPageByPageId($pageId) and
                is_a(
                    $applicationPage->getJazzeePage(),
                    '\Jazzee\Interfaces\DataPage'
                )
            ) {
                $applicant['pages'][] = $applicationPage->getJazzeePage()
                    ->formatApplicantArray($answers);
            }
        }
    ...
    public function formatApplicantDisplayArray(array $applicant)
    {
        ...
        foreach ($applicant['attachments'] as $attachment) {
            $base = $applicant['fullName'] .
                '_attachment_' . '_' . $attachment['id'];
            //remove slashes in path to fix an apache
            //issues with encoding slashes in redirects
            $base = str_replace(array('/', '\\'), 'slash', $base);
            $name = $base . '.pdf';
            $fileStore->createSessionFile($name, $attachment['attachmentHash']);
            $attachment['filePath'] =
                \Jazzee\Globals::path('file/' . \urlencode($name));
            if ($attachment['thumbnailHash'] != null) {
                $name = $base . '.png';
                $fileStore->createSessionFile(
                    $name,
                    $attachment['thumbnailHash']
                );
                $attachment['thumbnailPath'] =
                    \Jazzee\Globals::path('file/' . \urlencode($name));
            } else {
                $attachment['thumbnailPath'] = \Jazzee\Globals::path(
                    'resource/foundation/media/default_pdf_logo.png'
                );
            }
            $attachment['displayValue'] =
                "<a href='{$attachment['filePath']}'>" .
                "<img src='{$attachment['thumbnailPath']}' /></a>";
    ...
    public function formatApplicantPDFTemplateArray(array $applicant)
    {
        ...
        foreach ($answers as $pageId => $answers) {
            if ($applicationPage =
                ...
                is_a(
                    $applicationPage->getJazzeePage(),
                    '\Jazzee\Interfaces\PdfPage'
                )
            ) {
                $applicant['pages'][$applicationPage->getPage()->getId()] =
                    $applicationPage->getJazzeePage()
                        ->formatApplicantPDFTemplateArray($answers);
            }
        }
     ...
     * @param \Jazzee\CommonBundle\Entity\PDFTemplate $template
     */
     ...
     * Get template by Id
     * @param  integer                                 $id
     * @return \Jazzee\CommonBundle\Entity\PDFTemplate
     */
    ...
    {
        \Jazzee\Controller::getCache()
            ->delete(self::ARRAY_CACHE_PREFIX . $this->id);
    }
    ...
    public function setExternalIdValidationExpression($expression)
    {
        if (@preg_match($expression, null) === false) {
            throw new \Exception(
                "{$expression} is not a valid regular expression."
            );
        }
    ...
    public function validateExternalId($externalId)
    {
        ...
        $match = preg_match(
            $this->externalIdValidationExpression,
            $externalId
        );
