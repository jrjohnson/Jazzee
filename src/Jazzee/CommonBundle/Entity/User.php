...
class User
{
    ...
    protected $isActive;
    private $preferences;
    protected $defaultProgram;
    protected $defaultCycle;
    private $roles;
    protected $auditLogs;
    protected $displays;
    ...
    public function generateApiKey()
    {
        ...
        //So we get unique through uniquid and we get random by
        //prefixing it with a part of an MD5
        $seed =  md5(
            (mt_rand() * mt_rand() * $this->id) .
            $this->uniqueName . $this->lastName . $this->firstName
        );
        $this->apiKey = \uniqid($seed);
    }
    ...
    public function setApiKey($apiKey)
    {
        if (strlen($apiKey) < 32) {
            throw new \Exception(
                "Api key must be at least 32 charecters you passed: {$apiKey}"
            );
        }
        $this->apiKey = $apiKey;
    }
    ...
    {
        if (!is_null($this->preferences) and
            array_key_exists($applicationId, $this->preferences)
        ) {
     ...
     */
    public function isAllowed($controller, $action, Program $program = null)
    {
        foreach ($this->roles as $role) {
            if (($role->isGlobal() or
                $role->getProgram() == $program) and
                $role->isAllowed($controller, $action)
            ) {
                return true;
            }
        }
    ...
    public function getMaximumDisplayForApplication(Application $application)
    {
        $display = new \Jazzee\Display\Union();
        $hasProgramRole = false;
        foreach ($this->roles as $role) {
            if (!$role->isGlobal() and
                $role->getProgram()->getId() ==
                ...
                if ($roleDisplay = $role->getDisplay()) {
                    ...
                    $display->addDisplay(
                        new \Jazzee\Display\FullApplication($application)
                    );
                    //we interupt the loop here because once we have
                    //included a full display there is no reason to continue
                    break;
                }
            }
        }
