...
/**
 * ApplicationPage
 * Assocaites a Page with an Application.  
 * Allows the application to override many of the 
 * page properties for global pages
 *
...
class ApplicationPage
{
    ...
    public function setKind($kind)
    {
        ...
        if (!in_array($kind, $allowed)) {
            throw new \Exception(
                $kind . ' is not a valid application page kind.'
            );
        }
        if (($kind == self::SIR_ACCEPT or $kind == self::SIR_DECLINE) and
            !($this->getJazzeePage() instanceof \Jazzee\Interfaces\SirPage)
        ) {
            throw new \Exception(
                'Tried to set an SIR page kind, but ' .
                $this->getPage()->getType()->getClass() .
                ' does not implement \Jazzee\Interfaces\SirPage'
            );
        }
        $this->kind = $kind;
    }
    ...
    /**
     * Check constraints
     * Ensure there is no more than one application page of 
     * kind SIR_ACCPT or SIR_DECLINE
     */
    public function checkConstraints()
    {
        $kinds = array(
            self::SIR_ACCEPT  => 'SIR_ACCEPT',
            self::SIR_DECLINE => 'SIR_DECLINE'
        );
        foreach ($kinds as $kind => $name) {
            if ($this->kind == $kind) {
                $kindPages = $this->application->getApplicationPages($kind);
                foreach ($kindPages as $page) {
                    if ($page !== $this) {
                        throw new \Exception(
                            "{$this->getTitle()} and {$page->getTitle()} " .
                            "both have the kind {$name}.  This is not allowed."
                        );
                    }
                }
     ...
     * If this isn't a global page then store 
     * the instructions in Page and not here
     * @param string $value
     */
    ...
    public function getJazzeePage()
    {
        if (is_null($this->jazzeePage)) {
            ...
            if (!($class instanceof \Jazzee\Interfaces\Page)) {
                throw new \Exception(
                    $this->page->getType()->getName() . ' has class ' . $class .
                    ' that does not implement \Jazzee\Interfaces\Page interface'
                );
            }
