...
class Decision
{
    ...
    protected $id;
    protected $applicant;
    protected $nominateAdmit;
    protected $nominateDeny;
    protected $finalAdmit;
    protected $finalDeny;
    protected $offerResponseDeadline;
    protected $decisionViewed;
    protected $acceptOffer;
    protected $declineOffer;
    ...
    public function nominateAdmit($dateString = null)
    {
        if (!is_null($this->nominateDeny)) {
            throw new \Exception('Cannot record two preliminary decisions');
        }
    ...
    public function undoNominateAdmit()
    {
        if (!is_null($this->finalAdmit)) {
            throw new \Exception(
                'Cannot remove a preliminary decision if a final ' .
    ...
    {
        if (!is_null($this->finalAdmit)) {
            throw new \Exception('Cannot record two final decisions');
        }
    ...
    public function undoFinalAdmit()
    {
        if (!is_null($this->acceptOffer) and !is_null($this->declineOffer)) {
            throw new \Exception(
                'Cannot undo admit for an applicant with a offer response.'
            );
        }
    ...
    public function acceptOffer($dateString = null)
    {
        if (is_null($this->finalAdmit)) {
            throw new \Exception(
                'Cannot accept offer for an applicant who has not been admitted'
            );
        }
    ...
    public function declineOffer($dateString = null)
    {
        if (is_null($this->finalAdmit)) {
            throw new \Exception(
                'Cannot decline offer for an applicant ' .
     ...
     * @param  string $format optionally provide a format
     * @return array
     */
    ...
    public function can($status)
    {
        switch ($status) {
            case 'nominateAdmit':
                return (is_null($this->nominateAdmit) and
                        is_null($this->nominateDeny)
                );
        ...
        throw new \Exception("{$status} is not a valid decision status type");
    }
}
