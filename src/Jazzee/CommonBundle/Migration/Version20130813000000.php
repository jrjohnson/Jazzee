...
class Version20130813000000 extends \Doctrine\DBAL\Migrations\AbstractMigration
{
    ...
    public function up(\Doctrine\DBAL\Schema\Schema $schema)
    {
        ...
        $sql =
            'SELECT id,eShortString from element_answers WHERE element_id IN ' .
            '(SELECT id from elements where type_id IN ' .
            '(SELECT id from element_types where ' .
            'class="\\\Jazzee\\\Element\\\TextInput"' .
            '))';
        $fixShortString = $this->connection->prepare(
            'UPDATE element_answers SET eShortString=:newString WHERE id=:id'
        );
        $stmt = $this->connection->executeQuery($sql);
        $updated = 0;
        while ($row = $stmt->fetch()) {
            $newString = htmlentities(
                $row['eShortString'],
                \ENT_COMPAT,
                'UTF-8',
                false
        ...
        if ($updated > 0) {
            $this->write(
                "<info>Modified {$updated} TextInput elements for applicants " .
                "to encode their answers with HTML entities.</info>"
            );
        }
        ...
        $sql = 'SELECT id,eText from element_answers WHERE element_id IN ' .
                ...
                '))';
        $fixText = $this->connection->prepare(
            'UPDATE element_answers SET eText=:newString WHERE id=:id'
        );
        $stmt = $this->connection->executeQuery($sql);
        $updated = 0;
        while ($row = $stmt->fetch()) {
        ...
        if ($updated > 0) {
            $this->write(
                "<info>Modified {$updated} TextArea elements for applicants " .
                "to encode their answers with HTML entities.</info>"
            );
        }
    }
    ...
    public function down(\Doctrine\DBAL\Schema\Schema $schema)
    {
        ...
        if ($updated > 0) {
            $this->write(
                "<info>Modified {$updated} TextInput elements for applicants " .
                "to only escape <> in their answers.</info>"
            );
        }
