...
class Version20130630000000 extends \Doctrine\DBAL\Migrations\AbstractMigration
{
    ...
    public function postUp(\Doctrine\DBAL\Schema\Schema $schema)
    {
        ...
        $sql = 'SELECT li.* FROM element_list_items AS li ' .
            ...
            '(SELECT id FROM elements WHERE page_id IN (SELECT id from pages where type_id=' .
        ...
        $stmt = 'SELECT value FROM element_list_item_variables WHERE item_id=? AND name="code"';
        $fetchCode = $this->connection->prepare($stmt);
        $stmt2 = 'SELECT value FROM element_list_item_variables WHERE item_id=? AND name="searchTerms"';
        $fetchSearchTerms = $this->connection->prepare($stmt2);
        $total = count($rows);
        $newSchools = 0;
        foreach ($rows as $row) {
        ...
        $sql = 'UPDATE answers SET school_id = ' .
            ...
            'WHERE page_id IN (SELECT id from pages where type_id=' .
            '(SELECT id from page_types where class="\\\Jazzee\\\Page\\\Education"))';
        ...
        $rows = $this->connection->executeUpdate($sql);
        $this->write(
            "<info>Moved all the Known School page elements to the Education page's new single page form.  " .
            "Modified {$rows} rows</info>"
        );
        $sql = 'UPDATE element_answers set answer_id = ' .
            '(SELECT parent_id from answers where id = element_answers.answer_id) ' .
            'WHERE answer_id IN ' .
        ...
        $rows = $this->connection->executeUpdate($sql);
        $this->write(
            "<info>Migrated Applicant Known school Answers to the Education page.  Modified {$rows} rows</info>"
        );
        ...
        $this->write(
            "<info>Dis-associated all the applicant New School Answers from the Education Page.  " .
            "This will result in some applicant data loss and can be manually corrected in the DB.  Modified " .
            count($rows) . " answers.</info>"
        );
        $sql = 'SELECT id from pages where type_id=(SELECT id from page_types ' .
            'where class="\\\Jazzee\\\Page\\\Education")';
        $rows = $this->connection->executeQuery($sql)->fetchAll();
        $insertPageVars = $this->connection->prepare(
            'INSERT INTO page_variables (page_id, name, value) VALUES (?,?,?)'
        );
        $insertNewSchoolPage = $this->connection->prepare(
            'INSERT INTO pages (parent_id, title, fixedId, type_id) VALUES ' .
            '(?,?,?,(SELECT id from page_types WHERE class=?))'
        );
        ...
        foreach ($rows as $row) {
            ...
            $insertNewSchoolPage->execute(
                array($row['id'],
                'New School',
                \Jazzee\Page\Education::PAGE_FID_NEWSCHOOL,
                '\Jazzee\Page\Standard')
            );
        }
        $this->write(
            "<info>Removed existing child pages, created new school page, set page elements for " .
            "existing educaiton pages.  Modified " . count($rows) . " pages.</info>"
        );
        $sql = 'SELECT id from pages WHERE fixedId = 4 AND parent_id IN (SELECT id from pages where type_id=' .
            '(SELECT id from page_types where class="\\\Jazzee\\\Page\\\Education"))';
        $rows = $this->connection->executeQuery($sql)->fetchAll();
        $stmt3 = 'INSERT INTO elements (page_id, weight, fixedId, title, max, required, type_id) ' .
            'VALUES (?,?,?,?,?,?,(SELECT id from element_types WHERE class=?))';
        $insertPageElement = $this->connection->prepare($stmt3);
        foreach ($rows as $row) {
            $tie = '\Jazzee\Element\TextInput';
            $count = 1;
            $insertPageElement->execute(
                array($row['id'],
                $count++,
                \Jazzee\Page\Education::ELEMENT_FID_NAME,
                'School Name',
                255,
                1,
                $tie)
