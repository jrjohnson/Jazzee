...
class Version20130219000000 extends \Doctrine\DBAL\Migrations\AbstractMigration
{
    ...
    {
        parent::preUp($schema);
        $this->tmpDir = rtrim(sys_get_temp_dir(), \DIRECTORY_SEPARATOR) . \DIRECTORY_SEPARATOR . uniqid();
    ...
    public function up(\Doctrine\DBAL\Schema\Schema $schema)
    {
        ...
        $table->addColumn('thumbnailHash', 'string', array('length'  => 128, 'notNull' => false));
    ...
    public function postUp(\Doctrine\DBAL\Schema\Schema $schema)
    {
        ...
        foreach (scandir($this->tmpDir) as $item) {
            ...
            unlink($this->tmpDir . DIRECTORY_SEPARATOR . $item);
        }
        rmdir($this->tmpDir);
    }
    ...
    protected function storeElementAnswers()
    {
        ...
        foreach ($ids as $id) {
            ...
            $hash = \sha1($blob);
            $path = $this->tmpDir . \DIRECTORY_SEPARATOR . $hash;
            if (!file_exists($path)) {
                file_put_contents($path, $blob);
            }
            $inserts[$id] = $hash;
        }
        file_put_contents($this->tmpDir . \DIRECTORY_SEPARATOR . 'element_answers_inserts', \serialize($inserts));
    ...
    protected function storeAttachments()
    {
        ...
        file_put_contents($this->tmpDir . \DIRECTORY_SEPARATOR . 'attachment_inserts', \serialize($inserts));
    ...
    protected function storeFiles()
    {
        ...
        foreach ($ids as $id) {
            ...
            $rows = $this->connection->executeQuery(
                'SELECT id FROM element_answers WHERE eShortString=?',
                array($file['hash'])
            )->fetchAll(\PDO::FETCH_COLUMN);
            foreach ($rows as $id) {
        ...
        file_put_contents($this->tmpDir . \DIRECTORY_SEPARATOR . 'files_inserts', \serialize($files));
        $this->write("<info>Stored {$total} files.</info>");
    }

    protected function migrateElementAnswers()
    {
        $arr = unserialize(file_get_contents($this->tmpDir . \DIRECTORY_SEPARATOR . 'element_answers_inserts'));
        $fetchElement = $this->connection->prepare('SELECT eBlob FROM element_answers WHERE id=?');
        $checkHash = $this->connection->prepare('SELECT id FROM files WHERE hash=?');
        $insertFile = $this->connection->prepare(
            'INSERT INTO files (hash, encodedBlob, referenceCount) VALUES (?,?,?)'
        );
        $updateFile = $this->connection->prepare('UPDATE files set referenceCount=referenceCount+1 WHERE hash=?');
        $updateAnswer = $this->connection->prepare(
            'UPDATE element_answers SET eBlob = null, eShortString=? WHERE id =?'
        );
        $new = 0;
        $update = 0;
        $total = count($arr);
        foreach ($arr as $id => $hash) {
            $encodedBlob = base64_encode(file_get_contents($this->tmpDir . \DIRECTORY_SEPARATOR . $hash));
    ...
    {
        $arr = unserialize(file_get_contents($this->tmpDir . \DIRECTORY_SEPARATOR . 'attachment_inserts'));
        $checkHash = $this->connection->prepare('SELECT id FROM files WHERE hash=?');
        $insertFile = $this->connection->prepare(
            'INSERT INTO files (hash, encodedBlob, referenceCount) VALUES (?,?,?)'
        );
    ...
    {
        $files = unserialize(file_get_contents($this->tmpDir . \DIRECTORY_SEPARATOR . 'files_inserts'));
        $updateAttachment = $this->connection->prepare('UPDATE attachments SET attachment=? WHERE id =?');
        $updateElement = $this->connection->prepare(
            'UPDATE element_answers SET eBlob=? , eShortString=null WHERE id =?'
        );
        $elementAnswers = 0;
        $attachments = 0;
