...
class Version20130718000000 extends \Doctrine\DBAL\Migrations\AbstractMigration
{
    ...
    public function up(\Doctrine\DBAL\Schema\Schema $schema)
    {
        ...
        $stmt = 'DELETE FROM answers WHERE parent_id = ? AND id NOT IN ' .
                '(SELECT answer_id from element_answers)';
        $fixAnswer = $this->connection->prepare($stmt);
        $count = 0;
        foreach ($rows as $row) {
        ...
        if ($count > 0) {
            $this->write(
                "<info>Removed {$count} answers for Education pages where a " .
                "bug caused an extra child answer to be stored.</info>"
            );
        }
        $sql = 'SELECT applicant_id, answers.id, page_id, parent_id, applicants.firstName, applicants.lastName,' .
            ...
            '(SELECT title from application_pages where page_id=' .
            '(SELECT parent_id from pages where id=answers.page_id) and application_id = ' .
            '(SELECT application_id from applicants where id=applicant_id)) as applicationPageTitle, ' .
            '(SELECT name from programs where id=(SELECT program_id from applications WHERE id=' .
            '(SELECT application_id from applicants where id=applicant_id))) as programName ' .
        ...
        $statement = 'INSERT INTO element_answers (answer_id, element_id, position, eShortString) VALUES ' .
                '(:answerId, (SELECT id FROM elements WHERE page_id = :pageId AND fixedId=' .
                \Jazzee\Page\Education::ELEMENT_FID_NAME . '), 0, "Unknown School"), ' .
                '(:answerId, (SELECT id FROM elements WHERE page_id = :pageId AND fixedId=' .
                \Jazzee\Page\Education::ELEMENT_FID_COUNTRY . '), 0, "Unknown"), ' .
                '(:answerId, (SELECT id FROM elements WHERE page_id = :pageId AND fixedId=' .
                \Jazzee\Page\Education::ELEMENT_FID_CITY . '), 0, "Unknown")';
        $addUnknowSchool = $this->connection->prepare($statement);
        $addThread = $this->connection->prepare(
            "INSERT INTO threads (applicant_id, subject, createdAt) VALUES (:applicantId, :subject, :now)"
        );
        $addMessage = $this->connection->prepare(
            "INSERT INTO messages (thread_id, sender, text, createdAt, isRead) VALUES " .
            "(:threadId, :sender, :message, :now, 0)"
        );
        ...
        foreach ($rows as $row) {
            if (!in_array($row['applicant_id'], $applicantMessages)) {
                ...
                $message = "Dear {$row['firstName']} {$row['lastName']}, <br />" .
                    ...
                    "to reflect your information correctly, it will require your input. " .
                    "Please click on 'Back to Application,' then " .
        ...
        if ($messageCount > 0) {
            $this->write(
                "<info>Added {$messageCount} messages for applicants where a bug " .
                "caused Known School data to be dropped.</info>"
            );
        }
        if ($answerCount > 0) {
            $this->write(
                "<info>Modified {$answerCount} answers for Education pages where a bug caused " .
                "Known School data to be dropped.</info>"
            );
        }
    }
