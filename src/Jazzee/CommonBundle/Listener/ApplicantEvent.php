 ...
 * In order to updat the applicant metadata 
 * we listen for events to answers, payments, 
 * elementAnswers, etc
 * 
...
class ApplicantEvent
{
     ...
     * 
     * We have to do this operation usign onFlush 
     * so we can catch inserts, updated
    ...
    public function onFlush(\Doctrine\ORM\Event\OnFlushEventArgs $eventArgs)
    {
        ...
        $applicantMetadata = $entityManager
                ->getClassMetadata('Jazzee\Entity\Applicant');
        foreach ($entities as $entity) {
            ...
            switch (get_class($entity)) {
                ...
                case 'Jazzee\Entity\Message':
                    ...
                    $applicant = $entity->getPayment()
                        ->getAnswer()->getApplicant();
                    break;
            }
            if ($applicant and !$uow->isScheduledForDelete($applicant)) {
                $applicant->markLastUpdate();
                if ($uow->isScheduledForUpdate($applicant) or
                    $uow->isScheduledForInsert($applicant)
                ) {
