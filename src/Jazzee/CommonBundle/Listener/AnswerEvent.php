...
class AnswerEvent
{
    ...
    public function onFlush(\Doctrine\ORM\Event\OnFlushEventArgs $eventArgs)
    {
        ...
        $answerMetadata = $entityManager
            ->getClassMetadata('Jazzee\Entity\Answer');
        foreach ($entities as $entity) {
            ...
            while (!is_null($answer)) {
                if (!$uow->isScheduledForDelete($answer)) {
                    ...
                    if ($uow->isScheduledForUpdate($answer) or
                        $uow->isScheduledForInsert($answer)
                    ) {
    ...
    /**
     * When element answers are deleted they need 
     * to notify thier jazzee elements
     * 
     * @param \Doctrine\ORM\Event\OnFlushEventArgs $eventArgs
     */
